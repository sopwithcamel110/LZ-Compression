...
int main(int argc, char **argv) {
    ...
    if (help) {
        ...
        printf("   -i infile	   Input file of data to compress (default: stdin).\n");
        printf("   -o outfile	   Output file for compressed data (default: stdout).\n");
        return 0;
    }
    FileHeader header;
    memset(&header, 0, sizeof(header)); // Ensure all bits at &header are set to 0.
    struct stat sb;
    fstat(infile, &sb);
